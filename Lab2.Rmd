...
The alignment statistics from question #2 show the number of multimapped reads, which are reads that mapped equally well across multiple loci in the genome. The number of multimapped reads tells us how many reads map to *at least* 2 loci. In the STAR command, the parameter `--outFilterMultimapNmax` allows us to set the maximum number of loci that a read maps to. In the tutorial, we used a value of `1`, so we only kept uniquely mapped reads that aligned to `1` loci. 

a) Edit the STAR sbatch file and change the `outFilterMultimapNmax` parameter to **5**. Rerun the alignment and report the percentage of reads that are uniquely mapped in `Log.out.final`. Repeat for parameter values of **10, 15, 20**, each time reporting the percentage of multi-mapped reads. 
