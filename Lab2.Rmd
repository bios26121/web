...
Once logged in successfully, move to the scratch working directory:

```{bash,eval=F}
cd /scratch/midway2/CNETID
```

**In general, you should be running all commands while in your scratch directory unless otherwise stated.**
...
```{bash, eval=F}
ls /project2/xinhe/alan/Lab2_RNAseq_Processing/unmapped_reads/
```
...
```{bash,eval=F}
zcat /project2/xinhe/alan/Lab2_RNAseq_Processing/unmapped_reads/NA18856_argonne.fastq.gz | head -n 4    # print the first 4 lines of the file
```
...
```{bash, eval=F}
cp /project2/xinhe/alan/Lab2_RNAseq_Processing/fastqc.sbatch .
```
...
If the jod is running smoothly, the `Use` status will be `R`:
...
If for any reason the job fails, you can check `fastq_errors` file (in the same directory) for error messages.
...
```{bash, eval=F}
cp /project2/xinhe/alan/Lab2_RNAseq_Processing/align_reads.sbatch .
```
...
```{bash,eval=F}
cp /project2/xinhe/alan/Lab2_RNAseq_Processing/featurecounts.sbatch .
```
...
* What can you say about the average error rate as a function of base position? Hint: It is a problem associated with Illumina reads that we discussed in class. 
...
* Number of reads that are multi-mapped. What would you change (experimentally) to make this number lower?
* Percentage of reads unmappe. What is the primary reason for unmapped reads?
